...
function StatCard({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl p-5 sm:p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-brand-500/10 transition-all duration-500 hover:-translate-y-2 text-center">
        {/* Icon */}
        <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br ${stat.color} mx-auto mb-3 sm:mb-4 flex items-center justify-center text-white shadow-lg`}>
          {stat.icon}
        </div>

        {/* Number */}
        <div className="flex items-center justify-center gap-1 mb-1.5 sm:mb-2">
          <span className="text-3xl sm:text-5xl font-black text-brand-950 tabular-nums stat-counter">
            {count.toLocaleString("ar-SA")}
          </span>
          <span className="text-xl sm:text-2xl font-bold text-brand-500">{stat.suffix}</span>
        </div>
...
export default function StatsSection() {
  ...
  return (
    <section id="stats" ref={ref} className="relative py-16 sm:py-24 overflow-hidden">
      {/* Background */}
