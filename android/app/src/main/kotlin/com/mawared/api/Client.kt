...
object MawaredApiClientFactory {
    fun create(
        ...
        return MawaredApi(
            ...
            files = retrofit.create(FilesApi::class.java),
...
/**
 * Auto-stamps a UUIDv4 `Idempotency-Key` header on every POST/PATCH that
 * doesn't already carry one. The Retrofit interfaces that explicitly take
 * an `Idempotency-Key` (orders.create, payments.createIntent) win because
 * their value is set on the request before this interceptor fires.
 */
private class IdempotencyInterceptor : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        val req = chain.request()
        if (req.method !in MUTATING_METHODS) return chain.proceed(req)
        if (req.header("Idempotency-Key") != null) return chain.proceed(req)
    ...
    private companion object {
        val MUTATING_METHODS = setOf("POST", "PATCH")
    }
}
